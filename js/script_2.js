...
function load_word(target_number){
    ...
    if(target_number!=-1){
        ...
        for(let i=0;i<contents_queue;i++){
            ...
            contents_column=document.createElement('div');
            contents_column.name="contents_column";
            contents_column.id="content"+contentID;
            contents_column.className="contents_border"

            content_customID=1;//品詞の読み込み
            content_load(content_customID,i);

            content_customID=2;//訳語の読み込み
            content_load(content_customID,i)
...
function content_load(content_customID,i){//地獄

    switch(content_customID){
        case 1:{//クラス情報設定
